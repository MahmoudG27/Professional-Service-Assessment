    ...
    switch (c.operator) {
        ...
        case "containsAny":
            return Array.isArray(answer) && Array.isArray(c.value)
                ? c.value.some(v => answer.includes(v))
                : false;
        case "containsAll":
            return Array.isArray(answer) && Array.isArray(c.value)
                ? c.value.every(v => answer.includes(v))
                : false;
        case "contains":
...
// ================================================================
function calculateComplexityScore(scenario, answers) {
    let total = 0;
...
// ================================================================
function calculateConfidenceAndCompletion(scenario, commonAnswers, scenarioAnswers, commonQuestions) {
...
// ================================================================
export function evaluateScenario(scenario, commonAnswers, scenarioAnswers, commonQuestions = []) {
    const allAnswers = { ...commonAnswers, ...scenarioAnswers };
    const complexityScore = calculateComplexityScore(scenario, allAnswers);
    const { confidence, completionRate } = calculateConfidenceAndCompletion(scenario, commonAnswers, scenarioAnswers, commonQuestions);
    // Find complexity range
    const range = scenario.complexityRanges.find(r => complexityScore >= r.min && complexityScore <= r.max) ?? scenario.complexityRanges[scenario.complexityRanges.length - 1];
    ...
    const findings = Array.from(findingMap.values()).sort((a, b) => (severityOrder[a.severity] ?? 9) - (severityOrder[b.severity] ?? 9));
    ...
    const executiveSummary = {
        ...
        confidence,
        keyMessage: `Based on the assessment, this ${scenario.name.toLowerCase()} engagement ` +
...
function getImpactLabel(points) {
    if (points >= 30)
        return "High Impact";
    if (points >= 15)
...
export function getTopComplexityDrivers(scenario, answers, topN = 5) {
    ...
    for (const rule of scenario.scoringRules) {
        ...
        if (!question)
            continue;
        function addDriver(value, points) {
            if (points === 0)
                return;
