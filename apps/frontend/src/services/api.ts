...
export async function submitAndGenerateReport(
  companyProfile: CompanyProfile,
  scenarioId: string,
  scenarioAnswers: AssessmentAnswers
): Promise<ProfessionalServicesReport> {
  ...
  if (!reportData.success) throw new Error(reportData.error);
...
// Get PDF URL
export async function getPdfUrl(id: string) {
  const res = await fetch(`${BASE_URL}/assessment/${id}/pdf-url`, { headers });
  return res.json();
}

// Send report to client
export async function sendReport(id: string, email: string) {
